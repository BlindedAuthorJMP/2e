...
//	settings.condition_3_adjusted_likes = [12000, 9999999]; //1
  ...
  settings.likes_by = ['John','AncaD','Nick','Sarah','Lauren','Jane','Georgeee','Heather','Ky'];
  settings.Dislikes_by = ['John','AncaD','Sarah','Arjen','Jane','Georgeee','Laura','Heather','Ky'];
  }
    ...
    // When user receives likes
      ...
      //alert('WHEN USERS RECIVE LIKES FUNCTION')
  		...
  		for(var i=0; i<times.length; i++)
  		{
  			...
  			themsg = usernames[i] + " disliked your profile description";
...
//"likes": [10000, 12000, 40000, 80000, 120000], //5
        ...
        for(var i=0; i<times.length; i++)
        {
          if(times[i]== 12000 || times[i] == 35000 ||  times[i] ==  13333 || times[i] == 20000 || times[i] == 25000 || times[i] == 40000 || times[i] ==  9999999 || times[i] ==  9000 || times[i] ==  40000 || times[i] ==  38000 || times[i] ==  1000 || times[i] ==  87654 || times[i] == 76542 || times[i] ==  68791 || times[i] == 55248 ){
            setTimeout(function () {
            that.text(parseInt(that.text()) + 0);
            }, times[i]);
          }
